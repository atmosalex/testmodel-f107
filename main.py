...
class F107data(Dataset): #inherit the torch.utils.data.Dataset class
    def __init__(self, seqlen=5, use_f30_inputs = True, rollingtarget = True, use_f30_outputs = False, transform=None):
        self.transform = transform
        self.sl = seqlen
        self.use_f30_inputs = use_f30_inputs
        self.rollingtarget = rollingtarget
        self.use_f30_outputs = use_f30_outputs
        ...
        #10.7cm:
        #self.series_daily_y1 = pd.Series(np.interp(self.daily_epoch, f107data_epoch, f107data_y1))
        ...
        #pytorch tensors:
        # convert to tensors in __getitem__ instead
    ...
    def __getitem__(self, idx):
        ...
        if self.use_f30_inputs:
            inputs = np.vstack((f107inputs, f30inputs)).T
        else:
            inputs = np.expand_dims(f107inputs, 1)
...
def experiment_persistence(data, test_frac = 0.2):
    ...
    print(f"test period is {datetime.datetime.fromtimestamp(data.daily_epoch[test_idx])} to {datetime.datetime.fromtimestamp(data.daily_epoch[-1])}")
    fherror = []
    loss = nn.MSELoss()
...
dataset = F107data(seqlen = seqlen, rollingtarget=True, use_f30_inputs=True, use_f30_outputs=True) #81, 74 without, with f30
